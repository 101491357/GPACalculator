document.addEventListener("DOMContentLoaded", function() {
    window.toggleSemesterContent = function(semesterHeader) {
        const content = semesterHeader.nextElementSibling;
        if (content.style.display === "none" || content.style.display === "") {
            content.style.display = "block";
            semesterHeader.querySelector('span').innerHTML = '&#9652;'; // Up arrow
        } else {
            content.style.display = "none";
            semesterHeader.querySelector('span').innerHTML = '&#9662;'; // Down arrow
        }
    }
});
...
function updateCredits(input) {
    const course = input.value.toUpperCase();
...
function addSemester() {
    ...
    newSemesterDiv.innerHTML = `
        <h2 onclick="toggleSemesterContent(this)">Semester ${semesterCount} <span>&#9662;</span></h2>
